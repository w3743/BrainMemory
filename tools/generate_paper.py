...
heading('4.6  长期记忆自适应衰减模拟', 2)
body('为验证自适应衰减模型的有效性，设计了 30 天时间线模拟实验。'
     '模拟创建 7 条初始强度均为 0.6 的记忆，分配四种访问模式：高频实用（22 次/30 天）、'
     '定期引用（6 次）、间隔访问（5 次）、中度实用（4 次）、'
     '低频查询（2 次）、一次性使用（1 次）和噪音记忆（0 次）。'
     '强化函数：new = R + 0.35 × (1 − R)，每次访问向 1.0 移动 35%。'
     '衰减函数：effective_d = 0.02 × (2 − R)，弱者遗忘加速。')
...
make_table(
    ['记忆类型', '访问次数', 'Day 30 强度 R', '衰减率 d', '行为描述'],
    [['tech_stack\\n高频实用', '22', '0.998', '0.0010', '每日强化，强度几乎恒定'],
     ['user_pref\\n个人信息', '6', '0.845', '0.0033', '定期引用，稳定高分'],
     ['code_review\\n间隔重要', '5', '0.757', '0.0045', '间隔 8-11 天访问，适度衰减'],
     ['env_setup\\n中度实用', '4', '0.673', '0.0060', '偶尔查询，自然衰减'],
     ['bug_note\\n低频技术', '2', '0.485', '0.0110', '极少查询，强度下滑'],
     ['temp_task\\n一次性', '1', '0.275', '0.0148', '仅 Day 1 使用，30 天后大幅遗忘'],
     ['noise\\n噪音', '0', '0.229', '0.0200', '从未访问，自适应加速衰减']])
table_caption('表 6  30 天自适应衰减模拟结果')

body('结果表明，自适应衰减成功区分了不同访问频率的记忆：'
     '访问次数与最终强度呈正相关（r = 0.96），且弱者加速遗忘的机制使'
     '零访问记忆衰退更快。噪音记忆 30 天后 R=0.229（普通指数衰减为 0.329），'
     '衰减加速 30%，验证了"越弱越易忘"的设计目标。')

body('图 1 展示了 7 条记忆在自适应衰减下的强度演化曲线，图 2 为强度热力图。')

try:
    doc.add_picture(str(Path(__file__).resolve().parent / 'sim_charts' / 'strength_curves.png'),
                    width=Inches(5.5))
    table_caption('图 1  30 天记忆强度演化曲线（自适应衰减）。')
    doc.add_picture(str(Path(__file__).resolve().parent / 'sim_charts' / 'strength_heatmap.png'),
                    width=Inches(5.5))
    table_caption('图 2  记忆强度热力图。红色=低强度，绿色=高强度。单元格标注强度值。')
except Exception:
    body('（图表文件未找到，请运行 tools/simulate_longterm.py 生成。）')
...
body('本文提出了 MemBrain，一种基于类脑遗忘曲线与 FSRS 间隔效应理论的 LLM Agent 长期记忆系统。'
     '核心贡献总结如下：')
body('（1）数学模型：提出自适应衰减公式 effective_d = d × (2 − R)，'
     '使弱记忆加速遗忘，模拟人类"富者愈富"的记忆规律。'
     '强化函数 new = R + 0.35 × (1 − R) 渐进逼近 1.0，'
     '30 天模拟验证了访问频率与最终强度的正相关（r = 0.96）。'
     '废弃了 L1/L2/L3 分层方案，改用连续 R 值直接表示记忆质量。')
...
# ════════════════════════════════════════════════════════════
fix_fonts(doc)
output = os.path.expanduser('~/Desktop/MemBrain_v5.docx')
doc.save(output)
print(f'已生成: {output}')
